...
├── plugins/               # 用户插件目录 (120个插件，gitignored)
├── assets/                # 资源文件目录
├── temp/                  # 临时文件目录
├── logs/                  # 日志目录
├── node_modules/          # NPM依赖包
...
| `conversation.ts` | 对话管理器（仅 TeleBox 版；Next 使用 mtcute 对话 API） |
...
- **Node.js要求**: 24.x
- **TypeScript版本**: ^5.9.2
- **Telegram 库版本**: @mtcute/node ^0.31.0
- **协议**: LGPL-2.1-only
...
`plugins/` 目录包含120个用户插件示例，展示了TeleBox的各种功能实现。
