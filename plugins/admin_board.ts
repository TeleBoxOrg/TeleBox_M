...
import { getRawType, getParticipant, getUserId } from "@utils/entityTypeGuards";
...
function toTargetChat(entity: any): TargetChat {
  return {
    ...
    username:
      getRawType(entity) === "channel" && entity.username ? entity.username : null,
    isChannel: getRawType(entity) === "channel",
...
async function getAdminCollection(
  ...
  if (target.isChannel) {
    ...
    allAdmins = users.filter(
      (user: any) =>
        getRawType(user) === "user" && adminParticipantUserIds.has(String(user.id)),
    );
  }
...
async function collectAdminStat(
  ...
  const participant = getParticipant(user) as any;
...
async function demoteAdminInTarget(
  ...
  const inputUser = await client.resolvePeer(user.id);

  if (target.isChannel) {
    const inputChannel = await client.resolvePeer(target.entity.id);
    await client.call({
  ...
  await client.call({
    ...
    chatId: getUserId(target.entity),
    userId: inputUser,
...
async function findUserInChatParticipants(
  ...
  return users.find((user: any) => {
    if (getRawType(user) !== "user") return false;
