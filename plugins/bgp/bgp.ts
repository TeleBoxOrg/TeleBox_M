...
async function resolveTargetIP(
    ...
    if (trigger?.text) {
        const ipFromTrigger = extractIPFromText(trigger.text);
    ...
    if (msg.replyToMessage?.id) {
        ...
        if (r?.text) {
            const ipFromReply = extractIPFromText(r.text);
    ...
    cmdHandlers: Record<string, (msg: MessageContext, trigger?: MessageContext) => Promise<void>> = {
        bgp: async (msg, trigger) => {
            ...
            try {
                ...
                const rawArgs = msg.text.split(" ").slice(1);
                ...
                if (rawArgs[0] === "dns") {
                    ...
                    await msg.edit({ text: `🔍 正在查询DNS解析记录...` });
                ...
                await msg.edit({ text: `🔍 正在生成BGP路由图...` });
