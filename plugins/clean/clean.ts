...
class CleanPlugin extends Plugin {
  ...
  // 命令处理器
  cmdHandlers: { [key: string]: (msg: MessageContext) => Promise<void> };
  ...
  // 主命令处理器
  private async handleClean(msg: MessageContext): Promise<void> {
  ...
  // 处理删除账号清理
  private async handleDeletedClean(client: any, msg: MessageContext, parts: string[]): Promise<void> {
  ...
  // 处理拉黑/解封清理
  private async handleBlockedClean(client: any, msg: MessageContext, parts: string[]): Promise<void> {
    const action = parts[2]?.toLowerCase();
...
// 私聊已注销账号清理 - 修复版：直接移除整个对话
  private async cleanDeletedPM(client: any, msg: MessageContext, deleteDialogs: boolean = false): Promise<void> {
    ...
    try {
      ...
      for (const dialog of dialogs) {
        if (dialog.isUser && isUser(dialog.entity) && isUserDeleted(dialog.entity)) {
  ...
  // 群组已注销账号清理
  private async cleanDeletedMember(client: any, msg: MessageContext, cleanMembers: boolean = false): Promise<void> {
    const chat = msg.chat;
    if (!chat || getRawType(chat) === 'private') {
    ...
    for await (const participant of participants) {
      if (hasRawType(participant, 'user') && isUserDeleted(participant)) {
        deletedCount++;
        deletedUsers.push({ 
  ...
  // 拉黑用户清理
  private async cleanBlockedPM(client: any, msg: MessageContext, includeAll: boolean = false): Promise<void> {
  ...
  // 群组解封
  private async unblockMember(client: any, msg: MessageContext, includeAll: boolean = false): Promise<void> {
    const ct = getRawType(msg.chat);
    if (ct !== 'group' && ct !== 'supergroup' && ct !== 'channel') {
  ...
  private async sendError(msg: MessageContext, errorMsg: string): Promise<void> {
  ...
  private async updateBlockedProgress(msg: MessageContext, processed: number, total: number, 
  ...
  private async checkBanPermission(client: any, chatId: any): Promise<boolean> {
    try {
      ...
      if (hasRawType(participant, 'channels.channelParticipant')) {
        const participantObj = getParticipant(participant) as { adminRights?: { banUsers?: boolean }; _?: string } | undefined;
        if (hasRawType(participantObj, 'channelParticipantCreator')) return true;
        if (hasRawType(participantObj, 'channelParticipantAdmin')) return participantObj!.adminRights?.banUsers || false;
      }
      
      if (hasRawType(participant, 'messages.chatFull')) {
        const fullChat = (participant as unknown as { fullChat: unknown }).fullChat;
        if (hasRawType(fullChat, 'chatFull')) {
          const participants = (fullChat as unknown as { participants: unknown }).participants;
          if (hasRawType(participants, 'chatParticipants')) {
            const meParticipant = (participants as unknown as { participants: Array<{ userId: number; _?: string }> }).participants.find((p) => String(p.userId) === String(me.id));
            if (hasRawType(meParticipant, 'chatParticipantCreator') || hasRawType(meParticipant, 'chatParticipantAdmin')) {
              return true;
            }
          }
        }
