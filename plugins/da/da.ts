  ...
  try {
    while (true) {
      ...
      const searchResult = await client.call({
        ...
        peer: chatEntity,
        q: "", // 空查询搜索所有消息
        fromId: { _: "inputPeerSelf" }, // 关键：指定from_id为自己
        filter: { _: "inputMessagesFilterEmpty" }, // 不过滤消息类型
        minDate: 0,
        maxDate: 0,
        offsetId: offsetId,
        addOffset: 0,
...
// 主删除命令
const da = async (msg: MessageContext) => {
  ...
  const taskId = msg.chat.id.toString();
  ...
  try {
    ...
    let chatName = "未知群组";
    try {
      const chat = await client.getChat(msg.chat.id);
      if ("title" in chat) {
