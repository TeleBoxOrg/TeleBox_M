...
import { getRawObject, hasRawType } from "@utils/entityTypeGuards";
  ...
  private getStickerSetName(sticker: any): string | null {
    if (sticker.attributes) {
      for (const attr of sticker.attributes) {
        if (hasRawType(attr, 'documentAttributeSticker') && attr.stickerset) {
        ...
        try {
          ...
          if (document.attributes) {
            for (const attr of document.attributes) {
              if (hasRawType(attr, 'documentAttributeSticker')) {
