...
import { getRawType, getMessageMedia, getMessageDocument, getMessageEntities } from "@utils/entityTypeGuards";
...
• 回复 WebM + <code>${mainPrefix}rev h c</code> → 水平翻转 + 负片效果`;
	...
	private async handleReplyMessage(
		...
		const replyEntities = getMessageEntities(replyMsg);
	...
	private async handleTextReverse(
		...
		if (reversedEntities.length > 0) {
			try {
				...
				return;
      /* ignored */
				} catch (err) { console.error('[rev] 编辑消息失败:', err); }
		}
	...
	private async tryHandleMediaTransform(
		...
		const media = getMessageMedia(replyMsg);
	...
	private isSupportedMedia(media: any): boolean {
		...
		const mediaType = getRawType(media);
		if (mediaType === 'messageMediaPhoto') return true;
		if (mediaType === 'messageMediaDocument') {
			const doc = getMessageDocument({ media } as unknown as import("@mtcute/dispatcher").MessageContext);
	...
	private getFileNameFromDocument(doc: any): string | null {
		...
		for (const attr of doc.attributes) {
			if (getRawType(attr) === 'documentAttributeFilename') {
				return (attr as { fileName?: string }).fileName ?? null;
			}
		}
		return null;
	}

	private getMediaDoc(mediaObj: unknown) {
		return getMessageDocument({ media: mediaObj } as unknown as import("@mtcute/dispatcher").MessageContext);
	}

	private isGifMedia(media: any): boolean {
		if (getRawType(media) === 'messageMediaDocument') {
			const doc = this.getMediaDoc(media);
			const fileName = this.getFileNameFromDocument(doc);
