      ...
      if (args[0].toLowerCase() === "cancel") {
        ...
        try {
            await client.call({
              ...
              stickerset: { _: 'inputStickerSetShortName' as const, shortName: packName },
              hash: 0,
            });
    ...
    stickerInfo: { isAnimated: boolean; isVideo: boolean; isStatic: boolean }
      ...
      for (let i = 1; i <= 50; i++) { // Try up to 50 packs
          ...
          try {
              const result = await client.call({
                _: 'messages.getStickerSet',
                stickerset: { _: 'inputStickerSetShortName' as const, shortName: autoPackName },
                hash: 0,
              });
              if (result?._ === 'messages.stickerSet') {
                  if (result.set?.count < 120) {
  ...
  private async createStickerSet(
    ...
    try {
      await client.call({
        ...
        userId: { _: 'inputUserSelf' as const },
        title: title,
        shortName: packName,
        stickers: [{
