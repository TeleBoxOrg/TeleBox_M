...
async function collectMessages(
    ...
    for (const msg of msgs) {
        ...
        if (needManualFilter && normalizedFilterId) {
            const senderObj = msg.sender as unknown as { raw?: { id?: number | bigint } } | null;
            const msgSenderId = normalizeId(senderObj?.raw?.id ?? (msg.sender as unknown as { id?: number | bigint } | null)?.id);
        ...
        const senderUser = msg.sender as unknown as { firstName?: string; username?: string } | null;
        ...
        uai: async (msg: MessageContext) => {
            ...
            try {
                ...
                if (fwdFrom?.fromId) {
                    ...
                    const senderPeer = repliedMsg.sender as unknown as { raw?: { id?: number | bigint } } | null;
                    const senderId = senderPeer?.raw?.id ?? (repliedMsg.sender as unknown as { id?: number | bigint } | null)?.id;
                    if (senderId) {
