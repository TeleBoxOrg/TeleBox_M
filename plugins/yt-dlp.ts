...
async function downloadYtDlp(msg: MessageContext, isUpdate: boolean = false): Promise<void> {
...
async function ensureYtDlpExists(msg: MessageContext): Promise<void> {
...
async function handleUpdateCommand(msg: MessageContext): Promise<void> {
...
async function downloadAndUploadSong(msg: MessageContext, songQuery: string, preferredTitle?: string, preferredArtist?: string) {
...
    artistName = toSimplified(artistName);
    ...
    // finalSearchQuery / songTitle / artistName / albumName 均为用户输入或经 AI 解析，
    // 绝不能拼进 shell 字符串。
    const args: string[] = [
...
async function handleApiKeyCommand(msg: MessageContext, apiKey: string): Promise<void> {
    if (!apiKey) {
...
const yt = async (msg: MessageContext) => {
    try {
...
    cmdHandlers: Record<string, (msg: MessageContext) => Promise<void>> = { yt };
}
