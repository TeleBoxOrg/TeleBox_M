...
size: ${stat.size} bytes`
  ...
  if (name === "send_file") {
    ...
    await client.sendMedia(context.msg.chat.id, {
      type: "document",
      file: target,
      caption: asString(args.caption).trim() || `\u6587\u4EF6\uFF1A${import_path.basename(target)}`,
      fileName: import_path.basename(target),
      replyTo: context.msg.replyToMessage?.id
    });
    return {
      ok: true,
      title: "\u6587\u4EF6\u5DF2\u53D1\u9001",
...
function getConversationBaseKey(msg, scope) {
  const source = msg.chat?.id || msg.senderId || "global";
...
async function safeEdit(msg, text, options = {}) {
  ...
  try {
    ...
    try {
      ...
      if (options.html) {
        try {
          const sent = await msg.replyText(truncate2(safePlain), { linkPreview: false });
          return sent || msg;
        } catch {
        }
      }
...
async function safeReply(msg, text, options = {}) {
  ...
  try {
    ...
    if (options.html) {
      try {
        const sent = await msg.replyText(truncate2(plainFallback), { linkPreview: false });
        return sent || null;
      } catch {
      }
    }
...
\u5927\u5C0F\uFF1A${stat.size} bytes`
  ...
  async sendWorkspaceFile(msg, workspace, value) {
    ...
    await client.sendMedia(msg.chat.id, {
      ...
      fileName: import_path4.basename(target)
    });
    await showHtmlMessage(
      msg,
