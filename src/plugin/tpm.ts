...
async function search(msg: MessageContext) {
  ...
  try {
    ...
    const repoLink = `\n🔗 <b>插件仓库:</b> <a href="https://github.com/TeleBoxOrg/TeleBox-Next-Plugins">TeleBox-Next-Plugins</a>`;
  ...
  try {
    ...
    for (let i = 0; i < dbPlugins.length; i++) {
      ...
      try {
        ...
        if (!pluginRecord.url) {
          skipCount++;
          logger.info(`[TPM] 跳过更新插件 ${pluginName}: 无URL记录`);
          continue;
        }
        ...
        if (!fs.existsSync(filePath)) {
          skipCount++;
          logger.info(`[TPM] 跳过更新插件 ${pluginName}: 本地文件不存在`);
          continue;
        }
        ...
        if (currentContent === response.data) {
          skipCount++;
          logger.info(`[TPM] 跳过更新插件 ${pluginName}: 内容无变化`);
          continue;
        }
        ...
        fs.copyFileSync(filePath, backupPath);
        logger.info(`[TPM] 旧版本已备份到: ${backupPath}`);
    ...
    if (updatedCount === 0 && silent) {
      ...
      logger.info("[TPM] 更新跳过: 无变化，跳过 reload");
      const skipPeerId = statusMsg.chat?.id;
    ...
    if (silent) {
      if (updatedCount > 0) {
        try {
          ...
          logger.error("[TPM] silent reload failed:", e);
        }
      }
    } else {
      await reloadAndFinalize(statusMsg, finalText);
...
if (require.main === module) {
  ...
  if (args.length === 0 || args?.[0] !== "install" || args?.length < 2) {
    logger.info("Usage: node tpm.ts install plugin1 plugin2 ...");
  }
  installPlugin(args, {
